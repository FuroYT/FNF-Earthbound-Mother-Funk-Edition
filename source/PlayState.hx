...
class PlayState extends MusicBeatState
{
	...
	public var defaultHudZoom = 0.65;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'megalo':
				...
				var back:FlxSprite = new FlxSprite(-400, -500);
				back.setGraphicSize(Std.int(back.width * 1.2));
				back.frames = Paths.getSparrowAtlas('megaloFar');
				back.animation.addByPrefix('idle', 'backmove', 24, true);
				back.animation.play('idle');
				back.updateHitbox();
				add(back);

				var fg:FlxSprite = new FlxSprite(-400, -500).loadGraphic(Paths.image("megaloNear"));
				fg.setGraphicSize(Std.int(fg.width * 1.2));
				fg.updateHitbox();
				add(fg);

			case 'ness':
				var bg:FlxSprite = new FlxSprite(-400, -500).loadGraphic(Paths.image("nessBgReal4K"));
		...
		if (ClientPrefs.nesResolution)
		{
			strumLine = new FlxSprite(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, -105).makeGraphic(FlxG.width, 10);
			if(ClientPrefs.downScroll) strumLine.y = FlxG.height - 25;
			strumLine.scrollFactor.set();
	
			timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, -135, 400, "", 32);
			timeTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			timeTxt.scrollFactor.set();
			timeTxt.alpha = 0;
			timeTxt.borderSize = 2;
			timeTxt.visible = showTime;
			if(ClientPrefs.downScroll) timeTxt.y = FlxG.height + 95;
		} else {
		...
		if (ClientPrefs.nesResolution)
		{
			...
			healthBarBG.y = FlxG.height * 1.13;
			...
			healthBarBG.yAdd = -4;
			add(healthBarBG);
			if(ClientPrefs.downScroll) healthBarBG.y = -70;
	...
	function eventPushed(event:EventNote) {
		switch(event.event) {
			...
			case 'Dadbattle Spotlight':
				...
				dadbattleBlack.makeGraphic(Std.int(FlxG.width * 4), Std.int(FlxG.height * 4), FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		healthBarPercent = healthBar.percent;

		if (dad.curCharacter.toLowerCase() ==  'ness')
		{
			if (healthBar.percent > 80)
			{
				healthBar.createFilledBar(FlxColor.fromRGB(228, 0, 88), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]));
				healthBar.updateBar();
			} else {
		...
		//iconP1.x = Std.int((FlxG.width / 2) + 5); //405
		//iconP2.x = Std.int((FlxG.width / 2) - (5 + iconP2.width)); //243
		if (ClientPrefs.nesResolution)
		{
			iconP1.x = 405;
			iconP2.x = 243;
		...
		switch(eventName) {
			...
			case 'Add Camera Zoom':
				...
				case 'Camera Flash':
					...
					camHUD.flash(FlxColor.WHITE, Std.parseFloat(value1));
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (SONG.song.toLowerCase() == 'onett' && isStoryMode)
			{
				FlxG.save.data.unlockedMegalo = true;
				FlxG.save.flush();
			}
			playbackRate = 1;
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					MusicBeatState.switchState(GameVars.menuState);
